...
describe("WHERE functionality", () => {
  ...
  it("should work with joins and where clauses", () => {
    ...
    const sql = query.toString();
    expect(sql).toBe(
      "SELECT g.id AS game_id, g.name AS game_name, g.desc AS description FROM games AS g LEFT JOIN developers AS d ON g.game_id = d.game_id WHERE g.game_id = 1",
    );
  });
  ...
  it("should produce same result for $eq and direct value", () => {
    const query1 = queryBuilder.from<TableFields>("games", "g").where({ game_id: 1 }).select(["game_id", "game_name"]);
  ...
  describe("OR conditions", () => {
    ...
    it("should handle OR with joins", () => {
      ...
      const sql = query.toString();
      expect(sql).toBe(
        "SELECT g.id AS game_id, g.name AS game_name, g.desc AS description FROM games AS g LEFT JOIN developers AS d ON g.game_id = d.game_id WHERE (g.game_id = 1) OR (g.description LIKE '%action%')",
      );
    });
    ...
    it("should handle inline OR syntax with operators", () => {
      const query = queryBuilder
        ...
        .where({
          ...
          or: [{ game_name: { $like: "%Mario%" } }, { release_year: { $lt: 1990 } }],
        })
        .select(["game_id", "game_name"]);
