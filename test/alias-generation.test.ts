...
describe("Automatic alias generation", () => {
  ...
  it("should auto-generate aliases for joins", () => {
    ...
    const sql = query.toString();
    expect(sql).toMatch(
      /SELECT t\d+\.id AS game_id, t\d+\.name AS game_name, t\d+\.desc AS description FROM games AS t\d+ LEFT JOIN developers AS t\d+ ON t\d+\.game_id = t\d+\.game_id/,
    );
  });

  it("should auto-generate aliases for subqueries", () => {
    const subquery = queryBuilder.from<TableFields>("games").where({ release_year: { $gt: 2000 } });
  ...
  it("should allow mixing explicit and auto-generated aliases", () => {
    ...
    const sql = query.toString();
    expect(sql).toMatch(
      /SELECT g\.id AS game_id, g\.name AS game_name FROM games AS g LEFT JOIN developers AS t\d+ ON g\.game_id = t\d+\.game_id/,
    );
  });
  ...
  it("should handle multiple joins with auto-generated aliases", () => {
    ...
    const sql = query.toString();
    expect(sql).toMatch(
      /SELECT t\d+\.id AS game_id, t\d+\.name AS game_name, t\d+\.desc AS description, t\d+\.publisher AS publisher_name FROM games AS t\d+ INNER JOIN developers AS t\d+ ON t\d+\.game_id = t\d+\.game_id LEFT JOIN publishers AS t\d+ ON t\d+\.game_id = t\d+\.game_id/,
    );
  });
  ...
  it("should handle auto-generated table alias with explicit subquery alias", () => {
    ...
    const query = queryBuilder.from(subquery, "recent_games").select(["*"]);
