...
describe('LIMIT functionality', () => {
  ...
  it('should work with complex WHERE conditions', () => {
    ...
    const sql = query.toString();
    expect(sql).toBe("SELECT game_id, game_name FROM games AS g WHERE (g.release_year > 2000) OR (g.game_name LIKE '%Mario%') LIMIT 5");
  });
  ...
  it('should work with JOINs', () => {
    ...
    const sql = query.toString();
    expect(sql).toBe('SELECT game_id AS id, game_name AS name, description AS desc FROM games AS g LEFT JOIN developers AS d ON g.game_id = d.game_id WHERE d.game_id < 100 LIMIT 10');
  });
  ...
  it('should work with OR conditions', () => {
    ...
    const sql = query.toString();
    expect(sql).toBe("SELECT game_id, game_name FROM games AS g WHERE (g.release_year > 2020) OR (g.game_name LIKE '%Classic%') LIMIT 3");
  });
  ...
  it('should work with chained WHERE conditions', () => {
    ...
    const sql = query.toString();
    expect(sql).toBe("SELECT game_id, game_name FROM games AS g WHERE g.release_year > 2000 AND g.game_name LIKE '%Action%' LIMIT 8");
  });
  ...
  it('should work with field aliases', () => {
    ...
    const sql = query.toString();
    expect(sql).toBe('SELECT game_id AS id, game_name AS title, release_year AS year FROM games AS g WHERE g.release_year >= 2010 LIMIT 15');
  });
  ...
  it('should work with mixed array field selection', () => {
    ...
    const sql = query.toString();
    expect(sql).toBe('SELECT game_id, game_name AS title, release_year FROM games AS g WHERE g.game_id IN (1, 2, 3, 4, 5) LIMIT 3');
  });
