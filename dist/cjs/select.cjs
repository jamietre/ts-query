...
let Select = class Select {
    getSource(query) {
        if (query instanceof _query.BaseQuery) {
            return `${query.tableName} AS ${query.tableAlias}`;
        } else if (query instanceof _compoundQuery.CompoundQuery) {
        ...
        } else if (query instanceof _where.Where) {
    ...
    getWhereClause(query) {
        if (query instanceof _where.Where) {
